...
class Logo extends Component {
  ...
  constructor() {
    super();
    this.bind('handleClick', 'changeColorOnRandomSegment');
    this.state = {
      isSpinning: false,
      isUpsideDown: false
    };
  ...
  handleClick(event: Object) {
    ...
    TweenMax.to(this._container, 3, {
      ...
      ease: Power4.easeOut,
      onComplete: () => this.setState({isSpinning: false, isUpsideDown: !this.state.isUpsideDown})
    });
    this.changeColorOnRandomSegment();
  }

  changeColorOnRandomSegment(event: SyntheticEvent) {
  ...
  render() {
    ...
    return (
      <svg onMouseEnter={this.changeColorOnRandomSegment} onClick={this.handleClick} ref={c => this._container = c} className={containerClass} fill={fillColor} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 215 323">
